...
vamos fazer de forma recursiva
  ...
  como o algoritmo percorre apenas o caminho ate k ent a arvore o tempo é O(h).*/
...
void imprimirCaminhoK(ArvoreB* l,int k){
    if (l==NULL){
        return ;
    }
    ...
    if (l->id == k){
        ...
        printf(" - %d",l->id);
        imprimirCaminhoK(l->esq,k);

    }
    else if (l->id < k){
        printf(" - %d",l->id);
        imprimirCaminhoK(l->dir,k);
