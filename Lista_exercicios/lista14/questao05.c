...
/* A tarefa consiste em Procurar pelo elemento k na árvore e,
 se ele não estiver lá, encontrar o elemento da árvore que está mais próximo de k.
árvore que está mais próximo de k.
vamos fazer de forma recursiva
  Analise: a funçao que resolver essa questao é a elementoProx,
...
int elementoProx(ArvoreB* l,int k ,int melhor){
    if (l==NULL){
        return melhor;
    } 
    ...
    if (abs(l->id - k) < abs(melhor - k)){
        melhor=l->id;
    }

    if (k < l->id){
        return elementoProx(l->esq,k,melhor);
    }
    else{
        return elementoProx(l->dir,k,melhor);
    }
...
int main(){
    ...
    imprimir(r);
    int prox=elementoProx(r,15,r->id);
    printf(" %d ", prox);
